...
void rep_eps (TTV_Point T, FILE *feps, Image I){
	...
	fprintf(feps,"%.1f %.1f moveto ",T.tab[i].x+1,I.H-T.tab[i].y+1);

	while(i<T.nb){
		fprintf(feps,"%.1f %.1f %.1f %.1f %.1f %.1f curveto \n ",T.tab[i+1].x+1,I.H-T.tab[i+1].y+1 , T.tab[i+2].x+1,I.H-T.tab[i+2].y+1 , T.tab[i+3].x+1,I.H-T.tab[i+3].y+1);
		i=i+4;
	}
