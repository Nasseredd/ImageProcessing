...
void extraire_tous_contour_simplification(Image I, FILE *feps, double d){
	...
	*Masque = extraire_masque(I);
	...
	while (trouver_pixel_depart(*Masque,x,y) == true){
		...
		TTV_Point L = simplification_douglas_peucker_bezier3(C, 0, C.nb - 1, d, compteur);
	...
	printf("Le nombre de courbes de Bezier3 est de %d \n", *compteur);
...
TTV_Point simplification_douglas_peucker_bezier3(TTV_Point CONT, UINT j1, UINT j2, double d, int *compteur){
	...
	double n = j2 - j1;
	Bezier_3 B3 = approx_bezier3(CONT, j1, j2);
	...
	if (dmax <= d){
		*compteur = *compteur + 1;
		...
		L1 = simplification_douglas_peucker_bezier3 (CONT, j1, k, d, compteur);

		TTV_Point L2 = creer_TTV_Point_vide ();
		L2 = simplification_douglas_peucker_bezier3 (CONT, k, j2, d, compteur);
